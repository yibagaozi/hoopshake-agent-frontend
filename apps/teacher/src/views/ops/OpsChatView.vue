...
async function loadSessions() {
  try {
    sessions.value = pageItems(await opsChatApi.listSessions(0, 50))
  } catch (err) {
...
async function loadMessages(sessionId) {
  ...
  try {
    ...
    let page = 0
    for (;;) {
      const res = normalizePage(await opsChatApi.listMessages(sessionId, page, 100))
...
async function ensureSession() {
  ...
  const created = await opsChatApi.createSession({})
...
async function removeSession(s) {
  ...
  try {
    await opsChatApi.removeSession(s.sessionId)
    ...
    <div class="main-head">
      <div>
        <div class="head-title-row">
          <h2 class="page-title">运维助手</h2>
        </div>
        <p class="page-sub">用自然语言问系统状态与排障思路 · 只读诊断，不执行任何动作</p>
      </div>
    </div>
      ...
      <div class="conv">
        ...
        <div ref="bodyEl" class="conv-body">
          <div v-if="!messages.length && !loadingMsgs" class="greet">
            <div class="bubble ai">
              你好，我是运维助手。可以问我平台现状、边缘设备为什么离线、某个错误码是什么意思，
              我会查总览、业务量、Agent 表现、系统健康与设备状态来回答。我只给判断和建议，不会动任何配置。
            </div>
          </div>
        ...
        <div class="conv-input">
          <input
            v-model="input"
            :placeholder="messages.length ? '继续问运维助手…' : '比如：edge-03 为什么离线？50310 是什么？'"
            maxlength="2000"
            :disabled="streaming"
            @keyup.enter="send()"
          />
